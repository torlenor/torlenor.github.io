...
1) The players take turns sowing their seeds. The current player takes all the seeds from one of his pits and places them, one by one counter-clockwise into each of the following pits, including his own end zone pit, but not in the opponents end zone pit.
1) If the last sown seed lands in an empty house owned by the current player, and if the opposite house contains seeds, all the seeds in the pit were he placed the last seed and the seeds in the opposite pit belongs to the player and shall be placed into his end zone.
1) If the last sown seed lands in the player's end zone, the player can take an additional move.
1) When a player does not have any more seeds in his pits, the game ends and the opposing player can take all its remaining seeds and place it in its end zone.
1) The player with the most seeds in their end zone wins.
...
3) For every step $t=1,2,...,T$: Calculate the discounted reward/return $Q_t=\sum^\infty_{k=0}\gamma^kR_{t+k+1}$.
